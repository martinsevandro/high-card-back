...
export class RiotService {
   ...
   async getAccountByRiotId(name: string, tag: string, server: string) {
      ...
      try {
         ...
         if (response.status === 403) {
            throw new HttpException(
               'Acesso negado. Verifique sua API Key.',
         ...
         if (!response.data?.puuid) {
            throw new HttpException(
               'PUUID não encontrado na resposta.',
               response.status,
            );
         }
         ...
         throw new HttpException(
            err.response?.data?.status?.message || err.message,
   ...
   async getMatchDetails(puuid: string, server: string, matchId: string) {
      ...
      try {
         ...
         if (!playerStats) {
            throw new HttpException(
               'Jogador não encontrado na partida.',
               HttpStatus.NOT_FOUND,
            );
         }
         ...
         const duoStats = matchData.info.participants.find(
            (p: { puuid: string; subteamPlacement: number }) =>
               p.puuid !== puuid &&
               p.subteamPlacement === playerStats.subteamPlacement,
         );
         ...
         const skinPosition = defineSkinPosition(
            skins,
            playerStats.kills,
            playerStats.deaths,
            playerStats.assists,
