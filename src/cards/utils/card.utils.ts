...
export function defineRole(gameMode: string, teamPosition: string, role: string): string {
  if (gameMode === "CLASSIC") {
    switch (teamPosition) {
      case 'TOP': return 'top';
...
export function defineCorDaBordaFrontal(k: number, d: number, a: number): string {
...
export function defineCorVerso(k: number, d: number, a: number): string {
  const kda = (k + a) / Math.max(d, 1);
  if (kda < 2.0) return "linear-gradient(45deg, #633B1B, #965F32, #BB8947, #A0652E)";
  if (kda < 4.0) return "linear-gradient(45deg, #929292, #D1D1D1, #EBEBEB, #B1B1B1)";
...
export function getItemIcon(itemId: number): string | null {
  ...
  return itemsData[itemId] ? `https://ddragon.leagueoflegends.com/cdn/15.8.1/img/item/${itemId}.png` : null;
}

// augments icons (arena)
export function getAugmentIconUrl(augmentId: number): string | null {
  const augment = augmentsData.find(a => Number(a.id) === augmentId && a.iconSmall);
  return augment
